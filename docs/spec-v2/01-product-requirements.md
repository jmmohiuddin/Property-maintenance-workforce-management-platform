...
| --- | --- | --- | --- | --- |
| `OPEN-1` | ~~Field app: React Native or PWA?~~ **RESOLVED 2026\-08\-21.** | Owner \+ engineering | ~~Phase 3 start~~ — unblocked | **React Native (Expo), confirmed.** iOS Safari has no Background Sync at any version, and WebKit evicts unsynced storage after \~7 days of no interaction; the failure mode is silent data loss on a technician's phone. Note the framing above was wrong: the existing ADR 0004 had *already* chosen React Native and rejected a PWA, so this confirms it rather than reversing it. See [ADR 0009](../adr/0009-field-app-platform-confirmed.md). |
...
| `OPEN-11` | **Which SMS / WhatsApp provider?** `ATS-14` and `LEAD-2` both require a non\-email channel, and no transport exists beyond email \(`packages/notify` ships `ConsoleTransport` and `ResendTransport` only\). The `Channel` union already declares `sms` and `whatsapp` with nothing behind them. | Owner | `ATS-14` applicant messaging; `LEAD-2` emergency lead alerts | Choose a WhatsApp Business API account or an SMS gateway. Added 2026\-08\-21: this was a real blocking decision with no entry in this register, so it was invisible as a decision anybody owed. The interview\-logistics half of `ATS-14` is **not** blocked on it and is being built on the existing email transport, behind the same `Channel` abstraction. |
